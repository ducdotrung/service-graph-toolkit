...
const allowedProjects = process.env.SERVICE_GRAPH_ALLOWED_PROJECTS ? new Set(process.env.SERVICE_GRAPH_ALLOWED_PROJECTS.split(',').map(value => value.trim()).filter(Boolean)) : null;
const auditLog = process.env.SERVICE_GRAPH_AUDIT_LOG;
...
function audit(id) {
  if (!auditLog) return;
  appendFileSync(auditLog, `${new Date().toISOString()} caller=${process.env.SERVICE_GRAPH_CALLER || 'local'} project=${id} action=read\n`, { mode: 0o640 });
  if (statSync(auditLog).size > 1024 * 1024) {
    const retained = readFileSync(auditLog, 'utf8').trimEnd().split('\n').slice(-10000).join('\n');
    writeFileSync(auditLog, `${retained}\n`, { mode: 0o640 });
  }
}
function project(id) {
  if (allowedProjects && !allowedProjects.has(id)) throw new Error(`project not authorized: ${id}`);
